...
MINIMUM is really a bare minimum and means if you go lower you will almost certainly hit some problems.  Going higher is encouraged.  If you find any of these versions are no longer compatible (need raising) [[https://github.com/WireCell/wire-cell-toolkit/issues][make an issue]].
...
- The [[https://github.com/brettviren/winch][winch]] tool can build podman containers with WCT.
...
WCT comes with a large number of different types of tests which can be exercised via =ctest=.  The *doctest* target is the best compromise between coverage and speed.  In general, any failure is a bug but some failures require proper shell environment to find WCT data files.  Any failure of the *doctest* test is a bug.  To include tests, you must reconfigure.

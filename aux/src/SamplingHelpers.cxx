...
Dataset Aux::make_corner_dataset(const IBlob::pointer iblob, const bool drift, const double time_offset,
                            const double drift_speed)
{
    ...
    for (const auto& crossing : crossings) {
        ...
        if (drift) {
            double x_start = Aux::time2drift(iblob->face(), time_offset, drift_speed, iblob->slice()->start());
            double x_end = Aux::time2drift(iblob->face(), time_offset, drift_speed, iblob->slice()->start() + iblob->slice()->span());

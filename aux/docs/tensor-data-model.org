...
- The ~ID~ must be an integer no larger than 2^24.
...
The *datatype* of *flash* indicates a tensor representing a reconstructed summaries of activity consistent with *light* measurements.  The tensor carries no metadata.  The array is of type ~float32~ and may come in one of two shapes where ~N~ spans the number of "flashes" and the number of columns depends on if uncertainties are provided or not:

- ~(N, 5)~ :: One row per ~N~ flashes, each row: ~(t,x,y,z,q)~
- ~(N, 10)~ :: One row per ~N~ flashes, each row: ~(t,x,y,z,q,dt,dx,dy,dz,dq)~

Requirements on the content:

- The columns ~(t,x,y,z,q)~ provide central values and ~(dt,dx,dy,dz,dq)~ provide uncertainties on those CVs.

- The time ~t~ of *flashes* must be provided on the same clock as used by *light* measures.

- The ~x,y,z~ must be provided in global detector coordinates.

- An algorithm that produces no uncertainties must use the ~(N,5)~ form.

- An algorithm that produces any uncertainties must produce the ~(N,10)~ form.

- An algorithm that does not produce a particular CV must fill in a value outside the allowed domain.

- An algorithm that does not produce a particular uncertainty must fill in -1.

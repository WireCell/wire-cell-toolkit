...
| _facade_ | **Canonical flash facade expanded + matcher consolidated onto it.** Promoted the nested `Cluster::Flash` to a standalone `Clus::Facade::Flash` (additive: `idents()`/`pes(nchan)` accessors); added `Grouping::flashes()` enumerating all flashes via the single shared flashlight-join walk (also used by `Cluster::get_flash()`). `QLMatching` now reads flashes via `grouping->flashes()` and `Opflash` became a thin adapter over `Facade::Flash` (new facade ctor; dropped the dead tensor ctor + its `ITensor`/boost deps). uboone & SBND now share one flash representation. Backward-compatible (existing `get_flash()` consumers untouched); mc 40/40 byte-identical, data all 10 events. **uboone path verified** (`qlport/uboone-mabc.jsonnet`, the other `get_flash()` consumer via `retile_cluster`): event 5384/130/6501 runs clean, is deterministic run-to-run, and its `steiner`/`regular`/`shower_track`/`track_fit` bee outputs are byte-identical to the saved baseline — the flash-gated retile path is unchanged. (The `mc`/`vertices` bee outputs differ only cosmetically from a 6-week-old baseline — which DL-vertex endpoint carries the primary flag — a tagger/DL evolution with no flash code path, not this change.) |

| _opdump_ | **Q/L match persisted to the pctree; flash/op Bee dump moved into the all-APA MABC.** `QLMatching` now writes the match into the pctree (additive to the existing `flash` scalar / `cluster_t0`): a per-cluster `matched_flash_gid` scalar (globally-unique `anode·1e6 + per-APA-flash-index`, so the association survives the per-APA→all-APA merge), a per-cluster `flashpred` PC (the matched bundle's predicted per-channel PE = `op_pes_pred`), and a self-contained per-root `opflash` PC (`gid`/`time`/`ch`/`pe`, every flash's measured light). `Clus::PointTreeMerging` gained an opt-in `root_pcs_to_merge` list (default empty = unchanged; SBND all-APA opts in `["opflash"]`) — fixing the latent `auto`-copy no-op that silently dropped non-first-APA root PCs, but **only** for the named PCs so `ctpc_a*`/`dead_winds_a*`/`flash`/`light` keep their exact prior behavior. New `WireCell::Bee::Flashes` object; `Clus::MultiAlgBlobClustering` (`save_opflash`, default OFF; SBND `all_apa` ON) reads the merged `opflash` + per-cluster match at its **pre-clustering `img` dump point** (live clusters still == the per-APA matched clusters, ids == the `img` enumeration) and writes the `op` layer straight into `mabc-all-apa.zip`. Verified: mc op content equals the legacy `dump_light` op (`op_t`/`op_pes`/`op_peTotal`/`op_pes_pred`) with `cluster_id` now indexing `img-global` (0 dangling ids over 10 events; a matched flash resolves to the **same physical cluster** — identical centroid/npts — as the old op); both APAs present; data runs all 10 events. **Known degeneracy (flagged for later matching-algorithm tuning):** the pctree match is one-flash-per-cluster (MicroBooNE model), so the legacy per-*bundle* rows where one cluster matched two flashes (or a duplicate `(flash,cluster)` bundle) collapse to the single persisted last-wins match — ~5 display rows across 4/10 mc events; the persisted `flash` scalar/`cluster_t0` are byte-identical to baseline. |

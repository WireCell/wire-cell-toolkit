...
// return makes "re-adding a segment is not judged" fail.
//
// doc 77 round 1 (2026-08-24): the strict-mode refusal (knob
// graph_endpoint_strict) and its TEST_CASE were removed -- "must stay OFF"
// (pr/30 P8; pr/86:450).  See sbnd_xin/docs/77_knob-ledger.tsv.
...
TEST_CASE("pr30 P8 inconsistent endpoints are counted only when a connection is made")
{
    ...
    CHECK(boost::num_edges(g) == 1u);        // the edge is still made

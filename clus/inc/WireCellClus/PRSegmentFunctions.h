...
namespace WireCell::Clus::PR {
    ...
    struct TrackPidOptions {
        ...
        // forced to pdg 11 by segment_determine_shower_direction_trajectory.
        // That function has three branches keyed on endpoint degree; only
        // ONE of the three ("neither end is degree-1") tries track PID
        // first before discarding a non-electron result, and 18007's own
        // topology (one degree-1 end, one degree-2 end) takes a DIFFERENT,
        // fully unconditional branch that never calls track PID at all.
        // When true, segment_determine_shower_direction_trajectory runs
        // track PID BEFORE branch selection (uniformly, regardless of which
        // branch endpoint degree would otherwise pick) and trusts a
        // confident non-electron conclusion whenever
        // segment_dqdx_spares_electron_reclass independently confirms the
        // segment's own charge profile is not MIP-like -- same
        // reclassification-dQ/dx idiom shower_reclass_dqdx_guard already
        // gives the analogous kShowerTopology-flagged sites
        // (NeutrinoTrackShowerSep.cxx) and shower_topo_dqdx_guard gives the
        // topology test itself. false = legacy = byte-identical (the extra
        // track-PID call never runs at all when false).
        bool   shower_traj_dqdx_guard{false};
    };

#include "WireCellRoot/PdvdMagnifyTrackingVisitor.h"
...
WIRECELL_FACTORY(PdvdMagnifyTrackingVisitor, WireCell::Root::PdvdMagnifyTrackingVisitor,
                 WireCell::IConfigurable, WireCell::Clus::IEnsembleVisitor)

using namespace WireCell;
using namespace WireCell::Clus;

Root::PdvdMagnifyTrackingVisitor::PdvdMagnifyTrackingVisitor()
...
void Root::PdvdMagnifyTrackingVisitor::configure(const WireCell::Configuration& cfg)
{
...
WireCell::Configuration Root::PdvdMagnifyTrackingVisitor::default_configuration() const
{
    Configuration cfg;
...
Root::PdvdMagnifyTrackingVisitor::ChanScheme Root::PdvdMagnifyTrackingVisitor::chan_scheme() const
{
    ChanScheme cs;
...
void Root::PdvdMagnifyTrackingVisitor::visit(Clus::Facade::Ensemble& ensemble) const
{
    auto groupings = ensemble.with_name(m_grouping_name);
    if (groupings.empty()) {
        log->debug("PdvdMagnifyTrackingVisitor: no grouping '{}'", m_grouping_name);
        return;
    }
    ...
    log->debug("PdvdMagnifyTrackingVisitor: channel scheme nch=[{},{},{}] base=[{},{},{}]",
    ...
    if (!output_tf || output_tf->IsZombie()) {
        log->error("PdvdMagnifyTrackingVisitor: cannot open {}", outname);
        return;
    }
    ...
    log->debug("PdvdMagnifyTrackingVisitor: wrote {}", outname);
}

void Root::PdvdMagnifyTrackingVisitor::write_bad_channels(TFile* output_tf, Clus::Facade::Grouping& grouping,
    ...
    for (const auto& [apa, face] : apa_face_set) {
        for (int pind = 0; pind < 3; ++pind) {
            try {
                ...
                log->warn("PdvdMagnifyTrackingVisitor: failed to get dead channels for APA={}, face={}, plane={}: {}",
                          apa, face, pind, e.what());
            }
        }
    }

    log->debug("PdvdMagnifyTrackingVisitor: wrote {} entries to T_bad_ch", tree->GetEntries());
}
...
void Root::PdvdMagnifyTrackingVisitor::write_stm_trees(TFile* output_tf, Clus::Facade::Grouping& grouping) const
{
    ...
    log->debug("PdvdMagnifyTrackingVisitor: wrote T_stm_pass {} / T_stm_eval {} entries",
               t_pass->GetEntries(), t_eval->GetEntries());
}

void Root::PdvdMagnifyTrackingVisitor::write_trun(TFile* output_tf) const
{
...
void Root::PdvdMagnifyTrackingVisitor::write_proj_data(TFile* output_tf, Clus::Facade::Grouping& grouping,
                                                       const ChanScheme& cs) const
{
    auto tf = grouping.get_track_fitting(m_track_fitting_name);
    if (!tf) {
        log->warn("PdvdMagnifyTrackingVisitor: no TrackFitting in grouping slot '{}'", m_track_fitting_name);
        return;
    }
    ...
    if (fitted.empty() && per_cluster.empty()) {
        log->warn("PdvdMagnifyTrackingVisitor: fitted_charge_2d is empty");
        return;
    }
    ...
    auto emit = [&](std::map<int, std::map<std::pair<int, int>, Cell>>& local,
        ...
        const int channel = cs.global(plane_idx, apa, face, wt.first);
    ...
    log->debug("PdvdMagnifyTrackingVisitor: wrote T_proj_data with {} blocks", v_cluster_id.size());
}

void Root::PdvdMagnifyTrackingVisitor::write_t_rec_data(TFile* output_tf, Clus::Facade::Grouping& grouping,
                                                        const ChanScheme& cs) const
{
    auto nticks_map = grouping.get_nticks_per_slice();
    ...
    for (const auto* cluster : grouping.children()) {
        ...
        for (size_t i = 0; i < x.size(); ++i) {
            ...
            b_pu = cs.globalf(0, apa[i], face[i], pu[i]);
            b_pv = cs.globalf(1, apa[i], face[i], pv[i]);
            b_pw = cs.globalf(2, apa[i], face[i], pw[i]);
    ...
    log->debug("PdvdMagnifyTrackingVisitor: wrote {} entries to T_rec_charge", npoints);
}

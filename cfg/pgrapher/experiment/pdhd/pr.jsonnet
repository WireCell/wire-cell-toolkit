...
// are untouched.  The geometry objects (DetectorVolumes, PCTransformSet, the
// per-(anode,face) 'stepped' samplers, the T0 scope coordinates) are taken from
// the PDHD clustering module so the PR job sees exactly what the clustering
// built.
//
// PDHD deltas vs PDVD, each stated where it lives below:
//   * ONE global drift speed (PDVD has one per crate), so the clus module is
//     built with (time_offset, trigger_offset) only;
//   * 8 retiler samplers (4 anodes x 2 faces; only one face per anode images,
//     the other is a PDHD wall face -- the sampler config does not depend on
//     face, so both are emitted for lookup completeness, PDVD parity);
//   * pdhd_pr_fv: one BoxFiducial spanning both drift volumes, bounds = the
//     ACTIVE volume of pdhd/clus.jsonnet's dvm (x from the per-face FV_x
//     +-357.985 cm; y/z with the 15 cm space-charge inset REMOVED, exactly as
...
//   * nticks is an argument (PDHD readout window 6000, post-resample 5999);
...
function(output_dir='', runNo=1, subRunNo=1, eventNo=1,
    ...
    // these, so they must match for the round-trip identity gate.
    ...
    pr(anodes, dump=true, pipeline_names=[], tensor_outname='',
              // doc pdhd/03 (counterpart of doc pdvd/48): knob bag for the
              // check_stm_michel stage (verdict thresholds + PR-partition
              // keys); {} => C++ defaults.  Its partition knobs are FILTERED
              ...
              // doc pdvd/31 round 5, and it applies to PDHD IN FULL: ImproveCluster_2
              // (the Steiner stage's retiler) resolves a wire's channel by indexing
              // IWirePlane::channels() by WIRE index, and that list omits wrapped
              // continuations.  PDHD's induction planes hold 1148 wires on 800
              // channels per (anode, face) -- 400 segment-0, 400 segment-1, 348
              // segment-2 -- so 65 % of the wires on an imaging U/V plane are
              // continuations, against PDVD's 11.3 %.  (PDVD's own comment says
              ...
              trackfitting_config_file='pgrapher/experiment/pdhd/pdhd_track_fitting.json',
              ...
              // pdhd_pr_fv_margins), mirroring stm_consistent_fv above.
              ...
              // SAME pdvd_pr_fv + pdhd_pr_fv_margins as TGM/STM/FC, instead
              ...
              // pdhd_pr_fv_margins as cosmic_consistent_fv above -- same
       ...
       tcn_knobs={}):: {
        ...
        // permissive at every wall than pdvd_pr_fv + pdhd_pr_fv_margins below --
        ...
        // Box-model recombination at the PDHD drift field (uBooNE used 0.273 kV/cm).
        ...
        // PDHD: Modified Box with the SAME parameter set the dQ/dx reference
        // tables were generated with (convert_field.C: alpha 0.93, beta 0.212,
        // rho 1.38) at E = 0.4959 kV/cm -- the field the CALIBRATED drift
        // velocity 1.576 mm/us implies through the BNL LAr-properties mobility
        // parameterisation at T = 87.68 K (pdhd/stm/pdhd_transport.py).  Model
        // and tables therefore stay consistent, as on PDVD; SBND's A=1.0/B=0.255
        // is a different parameter set.
        local pdhd_box_recomb = {
            type: 'PracticalBoxRecombination',
            name: 'pdhd_box_recomb',
            data: { A: 0.93, B: 0.212, Efield: 0.4959, rho: 1.38, Wi: 23.6e-6 },
        },
        // Free-power Modified Box fitted to SBND stopping-track dQ/dx vs residual
        // range (docs/55 sec 7g).  PDHD has NO fitted power-box yet; the block
        // restates the SBND fit / C++ defaults so use_power_recomb=true is
        // well-defined.  PDHD production keeps it false.
        local pdhd_power_recomb = {
            type: 'PowerBoxRecombination',
            name: 'pdhd_power_recomb',
        ...
        // of per-face sensitive volumes, which excludes the CPA slab.
        // PDHD bounds, from pgrapher/experiment/pdhd/clus.jsonnet's dvm:
        //   x = +-357.985 cm, the per-face FV_x (dvm a0f0pA FV_xmin /
        //       a1f1pA FV_xmax), i.e. where the sensitive volume starts;
        //   y = 7.61 .. 606.0 cm and z = 0.234345 .. 462.297 cm, the ACTIVE
        //       volume -- dvm.overall's FV_y/FV_z are these inset by 15 cm for
        //       the clustering/Q-L containment filter, and the taggers take the
        //       active bounds plus their own margins instead.  This is exactly
        //       what SBND's sbnd_pr_fv and PDVD's pdvd_pr_fv do; PDVD later
        //       replaced the y/z faces with a measured curved surface (doc
        ...
        local pdhd_pr_fv_box = {
            ...
            data: {
                bounds: {
                    tail: { x: -357.985 * wc.cm, y: 7.61 * wc.cm, z: 0.234345 * wc.cm },
                    head: { x: 357.985 * wc.cm, y: 606.0 * wc.cm, z: 462.297 * wc.cm },
                },
            },
        },
        ...
        local cm_by_name = {
            ...
            // is not run by the PDVD Q/L chain, so there is no flash grouping to
            // undo.  It DOES need unmerge_assoc -- see below.
            //
            // doc pdvd/39 round 2: undo the isolated GROUPING.  cm.isolated()
            // (protodunevd/clus.jsonnet, group stage) physically MERGES a main
            // cluster with the small clusters that are near it but NOT connected
            // to it; the prototype only groups them (Clustering_isolated returns
            // main -> [(assoc, dis)] and leaves live_clusters untouched).  The
            // consequence, measured on evt 298595: cluster 109 reaches the STM
            // tagger as a 938-point body plus NINE fragments of 5-19 points at
            // 16-76 cm, ImproveCluster_2's retile then fabricates blobs along the
            // shortest path across those gaps (21% of that cluster's Steiner
            // points have no live 3D point within 3 cm; cluster 100 is 76%), and
            // the tagger fits the bridged whole as one track.  Same defect and
            // same cure as SBND (sbnd/clus.jsonnet unmerge_assoc, docs 50-52).
            //
            // Requires the provenance pair from the clustering job:
            // cm.isolated(save_assoc_id=true) AND save_assoc_cluster_id on the
            // MABC that writes the pctree (protodunevd/clus.jsonnet
            // save_assoc_id).  Without them the perblob PC is absent and the
            // visitor is inert -- it cannot invent what was never recorded.
            //
            // Position: after flag_mains and BEFORE steiner.  Before steiner
            // because separate() does not carry node-local PCs, so the split must
            // precede steiner_pc creation.  After flag_mains (owner's decision,
            // doc pdvd/39 sec 11) so the split-off fragments are removed from the
            // main's Steiner build and STM fit WITHOUT being promoted to mains
            // and given cosmic verdicts of their own.
            //
            // Not in the default pipeline_names => absent from the compiled
            // config => byte-identical.  Runner flag: -unmerge.
            unmerge_assoc: cm.unmerge_bundle(name='assoc', mode='real',
            ...
            tagger_check_stm: cm.tagger_check_stm(
                ...
                // counterpart here.
                fiducial=(if stm_consistent_fv then wc.tn(pdhd_pr_fv) else null),
                fv_tolerance=(if stm_consistent_fv then pdhd_pr_fv_margins else []),
            ...
            // doc pdhd/03 (counterpart of doc pdvd/48): the stopping-muon +
            // Michel stage.  Anchors on the STM tagger's accepted pass
            // (stm_pass / stm_fit local PCs, so save_stm_fit must be on = PDHD
            ...
            // beyond the visitor name (see pr_visitor below).  Only compiled
            // in when named in pipeline_names => the production (-stm) config
            // is byte-identical.
            ...
            tagger_check_neutrino: cm.tagger_check_neutrino(
                ...
                fiducial=(if neutrino_consistent_fv || cosmic_consistent_fv || nue_sp_consistent_fv then wc.tn(pdhd_pr_fv) else null),
                fv_tolerance=(if neutrino_consistent_fv || cosmic_consistent_fv || nue_sp_consistent_fv then pdhd_pr_fv_margins else []),
        ...
        local tagger_uses = (if std.member(pipeline_names, 'tagger_check_stm')
                            ...
                            + (if std.member(pipeline_names, 'tagger_check_tgm')
                               ...
                               || ((neutrino_consistent_fv || cosmic_consistent_fv || nue_sp_consistent_fv)
                                   && std.member(pipeline_names, 'tagger_check_neutrino'))
                               // doc pdhd/03: the STM+Michel stage names pdhd_pr_fv
                               // under the same stm_consistent_fv switch.
                               || (stm_consistent_fv
                                   && std.member(pipeline_names, 'check_stm_michel'))
                               then pdhd_pr_fv_uses else []),
        local bee_zip_path = evt_out_prefix + 'mabc-pr.zip',
        // doc pdhd/03 (counterpart of doc pdvd/48): the PR-tail Bee layers
        // (track_fit / shower_track / vertices / mc) render whichever stage
        // published the fitter+graph.  check_stm_michel publishes exactly like
        // tagger_check_neutrino, so only the visitor NAME changes; the gates
        // that used to test for tagger_check_neutrino now test for either.
        // With neither stage in pipeline_names both locals reduce to the
        // legacy values => the production (-stm) compiled config is
        // byte-identical.
        ...
        local mabc = g.pnode({
            ...
            data: {
                ...
                // steiner_terminals_only are C++ defaults ""/""/false, and the
                // whole block is present only when tagger_check_stm is in the
                // pipeline => the compiled config is byte-identical otherwise.

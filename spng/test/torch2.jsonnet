...
function(tools, debug_force_cpu=false, apply_gaus=true, do_roi_filters=false, do_collate_apa=false, do_mp_finding=false) {
    ...
    local device = if debug_force_cpu then "cpu" else "gpu",
        ...
        stacked_spng : {
            ...
            local w_stacker_mp_finding =  g.pnode({
                type: 'TorchTensorSetStacker',
                name: 'w_stacker_mp_finding',
                data: {
                    output_set_tag: 'w_stacked',
                    multiplicity:4,
                },
            }, nin=4, nout=1),
            local w_stacker_mp_finding_face1 =  g.pnode({
                type: 'TorchTensorSetStacker',
                name: 'w_stacker_mp_finding_face1',
                data: {
                    output_set_tag: 'w_stacked',
                    multiplicity:4,
                },
            ...
            local torch_to_tensors_mp_finding_face1 = [
                g.pnode({
                    type: 'TorchToTensor',
                    name: 'torchtotensor_mp_finding_face1_%d' % [anode.data.ident],
                    data: {},
                }, nin=1, nout=1) for anode in tools.anodes
            ],
            ...
            local tensor_sinks_mp_finding_face1 = [g.pnode({
                type: 'TensorFileSink',
                name: 'tfsink_mp_finding_face1_%d' % anode.data.ident,
                data: {
                    outname: 'testout_mp_finding_face1_apa%d.tar' % anode.data.ident,
                    prefix: ''
                },
            }, nin=1, nout=0) for anode in tools.anodes],
            ...
            local mp_finding_centers = torch_to_tensors + spng_decons + 
                    ...
                    collators_for_mp_finding + collators_for_mp_finding_face1 + torch_to_tensors_mp_finding +
                    torch_to_tensors_mp_finding_face1 +
                    mp_finding + mp_finding_replicators +
                    mp_finding_face1 + post_mp_stackers +
                    [w_stacker_mp_finding, w_stacker_mp_finding_face1],
            local mp_finding_edges = [
                    ...
                    g.edge(mp_finding_face1[0], torch_to_tensors_mp_finding_face1[0]),
                    g.edge(mp_finding_face1[1], torch_to_tensors_mp_finding_face1[1]),
                    g.edge(mp_finding_face1[2], torch_to_tensors_mp_finding_face1[2]),
                    g.edge(mp_finding_face1[3], torch_to_tensors_mp_finding_face1[3]),

                    g.edge(torch_to_tensors_mp_finding_face1[0], tensor_sinks_mp_finding_face1[0]),
                    g.edge(torch_to_tensors_mp_finding_face1[1], tensor_sinks_mp_finding_face1[1]),
                    g.edge(torch_to_tensors_mp_finding_face1[2], tensor_sinks_mp_finding_face1[2]),
                    g.edge(torch_to_tensors_mp_finding_face1[3], tensor_sinks_mp_finding_face1[3]),
            ],

            local mp_finding_edges_stacked = [
                    g.edge(fans_to_stacks, spng_decons[0], 0),
                    g.edge(fans_to_stacks, spng_decons[1], 1),
                    g.edge(fans_to_stacks, spng_decons[2], 2),
                    ...
                    g.edge(w_unstacker, w_stacker_mp_finding, 0, 0),
                    g.edge(w_unstacker, w_stacker_mp_finding_face1, 1, 0),
                    g.edge(w_unstacker, w_stacker_mp_finding, 2, 1),
                    g.edge(w_unstacker, w_stacker_mp_finding_face1, 3, 1),
                    g.edge(w_unstacker, w_stacker_mp_finding, 4, 2),
                    g.edge(w_unstacker, w_stacker_mp_finding_face1, 5, 2),
                    g.edge(w_unstacker, w_stacker_mp_finding, 6, 3),
                    g.edge(w_unstacker, w_stacker_mp_finding_face1, 7, 3),

                    g.edge(mp_finding_replicators[0], collators_for_mp_finding[0], 0, 0),
                    g.edge(mp_finding_replicators[1], collators_for_mp_finding[0], 0, 1),
                    g.edge(w_stacker_mp_finding, collators_for_mp_finding[0], 0, 2),
                    
                    g.edge(mp_finding_replicators[0], collators_for_mp_finding_face1[0], 1, 0),
                    g.edge(mp_finding_replicators[1], collators_for_mp_finding_face1[0], 1, 1),
                    g.edge(w_stacker_mp_finding_face1, collators_for_mp_finding_face1[0], 0, 2),
            ...
            ret : g.intern(
                ...
                outnodes=(if do_mp_finding then (tensor_sinks_mp_finding + tensor_sinks_mp_finding_face1) else tensor_sinks), #std.flattenArrays(tensor_sinks_unstacked),
                edges=(if do_mp_finding then mp_finding_edges_stacked else edges),
            ),
        }.ret,
}

...
def process_fodder_dense(npz_path, output_rec):
    """
    ...
    # Create HDF5 files for U (0) and V (1) views with new naming convention
    h5_file = h5py.File(output_rec, 'w')
...
def process_truth_full(npz_path, output_tru):
    """
    Process truth npz file and create HDF5 files for all views concatted together.
    ...
    # Create HDF5 files with new naming convention
    h5_file = h5py.File(output_tru, 'w')
    ...
    try:
        for event_num in sorted(events.keys()):
            ...
            # If any are missing that's bad
            if not all(i in event_data for i in range(4)):
                raise RuntimeError(f'Could not find all keys in input file {npz_path}, event {event_num}')

            # Risky assumption about the shape/order of chans/ticks
            full_data = np.concat([event_data[i] for i in range(4)])

            print(f"  Creating tru-0/{event_num}/frame_ductor0, shape={full_data.shape}")
            h5_file.create_dataset(f"{event_num}/frame_ductor0", 
                                data=full_data, 
                                compression='gzip')
...
if 'snakemake' in globals():
    ...
    output_rec = snakemake.output.rec
    output_tru = snakemake.output.tru
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        process_fodder_dense(str(fodder_file), output_rec)
        print(f"Created {output_rec}")
        
        print(f"\nProcessing truth file: {truth_file}")
        process_truth_full(str(truth_file), output_tru)
        print(f"Created {output_tru}")

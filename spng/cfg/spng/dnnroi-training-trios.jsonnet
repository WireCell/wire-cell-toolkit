...
function(input,
	 ...
	 triofile="dnnroi-training-trio.npz",
         triominq=0.0,
         schema="tensor",
         detname='pdhd',
         engine='Pgrapher',
    ...
    // subgraphs.splat() merges tpc.splat into the DepoFluxSplat data block, so
    // the trio parameters ride in there rather than needing a separate node.
    local tpc = det.tpcs[0] {
        splat+: {
            trio_file: triofile,
            trio_min_charge: wc.numberify(triominq),
        },
    };  
    ...
    local truth = {
        frame: pg.pipeline([
            sg.splat_frame(factor=irebin, scale=1.0/fscale, extra_name="_SPLAT"),
            io.frame_sink(truth_filename)
        ]),
        tensor: pg.pipeline([
            sg.splat_tensor(factor=irebin, scale=1.0/fscale, extra_name="_SPLAT"),

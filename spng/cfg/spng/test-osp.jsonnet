// This produces a final graph to input depos and output OSP signal waveforms.
...
function(input, output="", detname='pdhd', tpcids=[], engine='Pgrapher', device='cpu', verbosity=0)
    ...
    local sinks =[sinkf(name, digitize=false) for name in onames];
    local sink = pg.crossline(sinks);

    local guts = det_mod(det, controls.config).depos_to_osp;
    local graph = pg.shuntlines([source, guts, sink]);

...
2. Use =RayGrid::Coordinates::point_indices_view()= to get indices of the two
   V-wires that cross each U-wire just before the U-wire endpoints.

   - This function is called with ~view=3~ to pick up V-view (0/1 is horiz/vert
     views).

   - For the V-wire with the low V-index to physically cross a U-wire, we must
     round its pitch via "ceil" and for the high V-index we must "floor".  In
     general it is not trivial to predict which endpoint is on the low or high
     V-index end.  Find V-view indices for both endpoints with "floor", from
     them determine which is the low end and add 1 to these low indices.

3. Expand each pair of the =N= pairs of bounds indices into a closed list of
   indices that span this range.  See suggestion below on this step.
...
6. Plug final =Uindex= and =Vindex= into Ray Grid Coordinates to get a corresponding =Windex= tensor

7. Stack =Uindex=, =Vindex= and =Windex= to form =(N,3)= *cell basis tensor*.

...
2. Use =RayGrid::Coordinates::point_pitches()= to find tensor =wire_vrange= of shape
   =(NwireU, 2)= holding the pair of V-view wire indices closest to endpoints.

   - Note: we really want V-view indices for V-view wires crossing each U-view
     while =point_pitches()= can return indices beyond the end points.  Also,
     =point_pitches()= calculates pitches in all three views.  We really want a
     =point_pitches_view(points, view, round_method)= where =round_method= gives
     "round", "ceil", "floor".  We'd then want to "ceil" for the lower endpoints
     and "floor" for the upper.

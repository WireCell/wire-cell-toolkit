...
// This is forked by bv@bnl.gov from 1.85 and added to wire-cell-toolkit.
//
// This list gives changes, made ("+") rejected ("-") or unsure/future ("?").
//
// - [+] allow for setS vertex descriptor containers
//       - use a map instead of a vector for local to global map
//
// - [+] provide method to get graph level property bundle.
//
// - [-] construct root subgraph from existing instance of "base" graph?
//       - this breaks the subgraph model too much
//
// - [?] make m_graph and other data private?
// 
// - [ ] remove subgraph copy constructor as a copy results in "vestigial
//       origin" problem where the child copy knows the parent but the parent
//       does not know the child copy.  Changes to the copy can propagate to the
//       parent but changes to other children to not propagate to the copy.
//       - for now, just add warning comment in the code.
//
// - [ ] Implement remove_vertex().  This is particularly thorny for vertices in
//       vecS as a remove invalidates all unrelated descriptors.  Use of setS
//       may be feasible.
...
public:
    ...
    typedef typename graph_bundle_type<Graph>::type graph_bundle_type;
    ...
    // Intern a "bare" base graph by copy.
    subgraph(const graph_type& base) : m_graph(base.m_graph), m_parent(0), m_edge_counter(0) {}
    ...
    // copy constructor - Warning, the copy is only half "in" the subgraph tree.
    // The copy knows about the original's parent but the parent does not know
    // about the copy.  The recur_up methods do what you'd expect but the
    // recur_down methods will not visit the copy.
    ...
    {
        return is_root() ? u_local : m_global_vertex.at(u_local);
    }
    ...
    const graph_bundle_type& graph_bundle() const
    {
        return m_graph[boost::graph_bundle];
    }
    graph_bundle_type& graph_bundle() 
    {
        return m_graph[boost::graph_bundle];
    }
...
private:
    ...
    typedef std::map< vertex_descriptor, vertex_descriptor > GlobalVertexMap;
...
public: // Probably shouldn't be public....
    ...
    GlobalVertexMap m_global_vertex; // local -> global
    ...
    typename subgraph< G >::vertex_descriptor u_global, subgraph< G >& g)
{
    ...
    if (!exists_local)
    {
        ...
        g.m_global_vertex[u_local] = u_global;
...
namespace detail
{
        ...
        subgraph< G >& g)
    {
        ...
        if (g.is_root())
        {
            ...
            // g.m_global_vertex.push_back(u_global);
            g.m_global_vertex[u_global] = u_global;
        }
        else
        {

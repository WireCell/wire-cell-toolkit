...
Channels are considered "live" if they are functioning properly or they are "dead".  A live channel may produce zero signal for some period of time.  That is interpreted as measuring an absence of activity.  A dead channel may produce non-zero signal for some period of time.  That is interpreted as inconclusive and ignored.  A channel may change between live and dead state throughout a given frame.

There are many ways to determine "live" or "dead".  Statically configured "bad channel lists" or dynamically determined regions of "badness" are two general ways.  These judgments on the space of channel and tick are represented by a "bad" channel mask map provided in an ~IFrame~ /eg/ as constructed by ~ChargeErrorFrameEstimator~.

Given the tomographic nature of wire-readout it is not feasible to deal with "dead" channels with some kind of ficducial cut.  Instead, the distinction of "live" vs "dead" channels are propagated through the WC imaging stage.  This leads to seven, mutually-exclusive types of blobs:

- *3-view live blobs* formed from three views each providing live channels.
- *2-view live blobs* as above but one view consists of dead channels.  There are three sub-types of 2-view live blobs, one for each plane supplying the dead channels.
- *2-view dead blobs* or simply *dead blobs*.  The measures of two views consist of dead channels.  Again, there are three sub-types where depending on which view provides the live view.

Both live and dead blobs are interpreted as "likely contains ionization signal" while any volume not contained by a blob is considered "likely does not contain ionization signal".  
2-view live blobs are largely considered as somewhat more ambiguous but otherwise generally similar to 3-view live blobs.   Dead blobs are used differently than live blobs.  For example, later clustering may allow two otherwise disconnected live blobs to become connected if the path between them goes through a dead blob but not form a connection between those two live blobs if no dead blob intercedes.

The ~GridTiling~ component that performs tiling is ignorant of any of this live vs dead interpretation.  It merely consumes the /activity map/ of an ~ISlice~.  This maps a channel to a charge value and a charge uncertainty.  The amount of charge is ignored in tiling and only the existence of the channel in this map and knowledge of the locations of the wires feeding each channel is considered in tiling.  

An instance of the component ~MaskSlices~ is added as a predecessor to ~GridTiling~ in the DFP graph.  ~MaskSlices~ applies a live/dead interpretation in the process of consuming an input ~IFrame~ and of preparing an output stream of ~ISlice~ instances and in particular generating their activity maps.  A signal sample from a live channel that is within the span of slice being created is added to that channels entry in the activithy map.  A signal value is taken from one named set of tagged traces and the signal uncertainty from another.  Any channel that is determined to be "dead" in a slice has its entry in the activity map set to a special activity of $0 \pm 10^{12}$, by convention.  This determination of "dead" may override an otherwise previously counted "live" channel.

A ~MaskSlices~ is further configured with three lists of plane indices:

- ~active_planes~ :: the planes considered to generally be alive
- ~masked_planes~ :: planes to ignore when checking for entries in the "bad" channel mask map.
- ~dummy_planes~ :: planes for which all channels are set to the special activity value
